...
volumes:
  ...
  {{- range $podVolumes -}}
  - name: {{ .name }}
    {{- toYaml .volumeTemplate | nindent 4 }}
  {{- end }}
  {{- end }}
...
containers:
  - name: {{ $containerName }}
    ...
    {{- if eq $type "service" }}
    livenessProbe:
      {{- toYaml (default .Values.livenessProbe .pod.livenessProbe) | nindent 6 }}
    readinessProbe:
      {{- toYaml (default .Values.readinessProbe .pod.livenessProbe) | nindent 6 }}
    ports:
    ...
    {{- end }}
    resources:
      {{- toYaml (default .Values.resources .pod.resources) | nindent 6 }}
    volumeMounts:
      {{- if and .Values.datadog .Values.datadog.enabled }}
      - name: apmsocketpath
